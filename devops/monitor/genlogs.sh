...
grep -h INFO  ~/${t}/logs/logfile.txt* | grep -v SMBConnection | grep -v watermarking | grep -v urllib3 | grep -v AuthN | grep -v accountperms | grep -v delete| perl -pe 's/\[(.*?) (.*?)\] (\w+) \[(.*?)\] (.*?) :: (.*?) :: (.*?)( :: .*?)?/${t}\t\1\t\2\t\3\t\4\t\5\t\6\t\7\t\8/'  > ${t}.temp1
grep -h DEBUG ~/${t}/logs/logfile.txt* | grep -v SMBConnection | grep -v watermarking | grep -v urllib3 | grep -v 'could not authenticate' | perl -pe 's/\[(.*?) (.*?)\] (\w+) \[(.*?)\] User: (.*) authenticated with Host: (.*)/${t}\t\1\t\2\t\3\t\4\t\5\t\6/' >> ${t}.temp1
