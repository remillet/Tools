...
def csv2cspace(xmlTemplate, csvDataDict, authority, config):
    try:
    ...
    uri = '%s/items' % authority

    messages = []
    messages.append("posting to cspace REST API...")
    payload = createXMLpayload(xmlTemplate, csvDataDict, INSTITUTION)
    # print payload
    (url, data, CSID, elapsedtime) = postxml('POST', uri, realm, protocol, hostname, port, username, password, payload)
    return [CSID, elapsedtime]
...
def getRecords(rawFile):
    ...
    try:
        ...
        for rowNumber, row in enumerate(csvfile):
            ...
            #    records[row[0]] = {}
            rows.append({'termDisplayName':row[0]})
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 6:
        print('%s <csv input file> <config file> <mapping file> <template> <output file>') % sys.argv[0]
        sys.exit()

    print header
    print "CSV2CSPACE: input  file:      %s" % sys.argv[1]
    print "CSV2CSPACE: config file:      %s" % sys.argv[2]
    print "CSV2CSPACE: authority & csid: %s" % sys.argv[3]
    print "CSV2CSPACE: template:         %s" % sys.argv[4]
    print "CSV2CSPACE: output file:      %s" % sys.argv[5]
    print header

    try:
        csvRecords, lines = getRecords(sys.argv[1])
        print 'CSV2CSPACE: %s lines and %s records found in file %s' % (lines, len(csvRecords), sys.argv[1])
        print header
        if lines == -1:
            print 'CSV2CSPACE: Error! %s' % csvRecords
            sys.exit()
    except:
        raise
        print "CSV2CSPACE: could not read CSV rows"
        sys.exit()

    try:
        config = getConfig(sys.argv[2])
        print "CSV2CSPACE: hostname        %s" % config.get('connect', 'hostname')
        print "CSV2CSPACE: institution     %s" % config.get('info', 'institution')
        print header
    except:
        print "CSV2CSPACE: could not get cspace server configuration"
        sys.exit()

    try:
        authority = sys.argv[3]
    except:
        print "CSV2CSPACE: could not get authority and csid"
        sys.exit()
    ...
    try:
        ...
        print "CSV2CSPACE: could not get template"
        sys.exit()

    try:
        outputfh = csv.writer(open(sys.argv[5], 'wb'), delimiter="\t")
    except:
        print "CSV2CSPACE: could not open output file for write %s" % sys.argv[5]
        sys.exit()

    successes = 0
    recordsprocessed = 0
    for i, csvData in enumerate(csvRecords):

        try:
            cspaceElements = csv2cspace(xmlTemplate, csvData, authority, config)
            print "CSV2CSPACE: csid: %s, %8.2f" % tuple(cspaceElements)
            if cspaceElements[0] != '':
                successes += 1
            outputfh.writerow(cspaceElements)
        except:
            print "CSV2CSPACE: create failed for record %s" % i
            print cspaceElements
            # raise
        recordsprocessed += 1

    print header
    print "CSV2CSPACE: %s records processed, %s successful PUTs" % (recordsprocessed, successes)
    print header