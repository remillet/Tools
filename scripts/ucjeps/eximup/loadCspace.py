...
def createXMLpayload(template, values, institution):
    ...
    for v in values.keys():
        payload = payload.replace('{' + v.lower() + '}', escape(values[v]))
    # get rid of remaining unsubstituted template variables
    payload = re.sub('(<.*?>){[^<]*}(<.*>)', r'\1\2', payload)
    return payload


def fims2cspace(xmlTemplate, objectNumber, fimsDataDict, config):
    try:
    ...
    if objectNumber == [] or objectNumber is None:
        ...
        fimsDataDict['objectnumber'] = objectNumber
...
if __name__ == "__main__":
    ...
    for bcid, fimsData in fimsRecords.items():
        ...
        try:
            cspaceElements = fims2cspace(xmlTemplate, bcid, fimsData, config)
            ...
            #raise
            print "FIMS2CSPACE: create failed for objectnumber %s, %8.2f" % (bcid, (time.time() - elapsedtimetotal))
            # raise
        recordsprocessed += 1
